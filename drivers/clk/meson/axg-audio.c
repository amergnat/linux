...
struct clk_regmap aud_##_name = {					\
	...
	.hw.init = &(struct clk_init_data) {				\
		...
		.parent_hws = (const struct clk_hw *[]) { &_pname.hw }, \
		.num_parents = 1,					\
...
#define AUD_MUX_PTR(_name, _reg, _mask, _shift, _dflags, _pnames, _iflags) \
struct clk_regmap aud_##_name = {					\
	.data = &(struct clk_regmap_mux_data){				\
		.offset = (_reg),					\
		.mask = (_mask),					\
...
#define AUD_TDM_PAD_CTRL(_name, _reg, _shift, _parents)			\
	AUD_MUX_PTR(tdm_##_name, _reg, 0x7, _shift, 0, _parents,	\
		CLK_SET_RATE_NO_REPARENT)

static const struct clk_hw *mclk_pad_ctrl_parent_hws[] = {
	&aud_mst_a_mclk.hw,
	&aud_mst_b_mclk.hw,
	&aud_mst_c_mclk.hw,
	&aud_mst_d_mclk.hw,
...
static AUD_TDM_PAD_CTRL(mclk_pad_0, AUDIO_MST_PAD_CTRL0, 0,
			mclk_pad_ctrl_parent_hws);
static AUD_TDM_PAD_CTRL(mclk_pad_1, AUDIO_MST_PAD_CTRL0, 4,
			mclk_pad_ctrl_parent_hws);

static const struct clk_hw *lrclk_pad_ctrl_parent_hws[] = {
	&aud_mst_a_lrclk.hw,
	&aud_mst_b_lrclk.hw,
	&aud_mst_c_lrclk.hw,
	&aud_mst_d_lrclk.hw,
